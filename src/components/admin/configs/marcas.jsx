...
const AccionesJS = () => {
  ...
  const onClickAction = async (e, ind, params) => {
    ...
    if (ind === "delete") {
      const res = await APIConsultas.marcas.DELETE(params.id);
      if (res) {
        getData();
  ...
  const edit = async (value) => {
    const res = await APIConsultas.marcas.UPDATE(value.id, value.value);
  ...
  const addCateg = async (e) => {
    e.preventDefault();
    if (name.length > 1) {
      const resp = await APIConsultas.marcas.ADD(name);
      if (resp) {
        getData();
        handleClose();
  ...
  return (
    <>
      <article className="w-full ">
        <div className="flex flex-col gap-4 justify-center px-5 py-6 shadow-sm rounded bg-black">
        ...
        <Box sx={style}>
          ...
          <form onSubmit={addCateg} className="pt-4">
            <div className="grid grid-cols-1 w-full">
              ...
              <input
                className="px-3 h-10 rounded-lg border-2 border-black mt-1 focus:border-primary outline-none"
                id={name}
                type="text"
                value={name}
            ...
            <div className="w-full text-center pt-6">
              <input
                className="w-full px-8 py-1 bg-primary rounded-xl font-bold text-white cursor-pointer"
                type="submit"
                value="Confirmar"
              />
            </div>
